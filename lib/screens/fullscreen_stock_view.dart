...
class _FullScreenStockViewState extends State<FullScreenStockView> {
  ...
  void _populateStocks() {
    ...
    for (int i = 0; i < 100; i++) {
        ...
        double changeVar = widget.mode == ViewMode.Risers ? Random().nextDouble() * 5 : -Random().nextDouble() * 5;
        
        _displayStocks.add(Stock(
      ...
      setState(() {
         for (int i=0; i<_displayStocks.length; i++) {
            ...
            _displayStocks[i] = Stock(
              ...
              name: s.name,
              price: newPrice,
              changeRate: s.changeRate + (change / s.price) * 100 // simplistic approx
            );
         }
      });
                    ...
                    child: LayoutBuilder(
                      builder: (context, constraints) {
                        ...
                        return GridView.builder(
                          physics: const NeverScrollableScrollPhysics(), // Disable scrolling to force fit
                          ...
                          itemBuilder: (context, index) {
                             if (index >= _displayStocks.length) return const SizedBox();
                             final stock = _displayStocks[index];
                             final color = _flashColors[stock.symbol] ?? Colors.transparent;
                                   ...
                                   Expanded(
                                     ...
                                     child: Text(
                                       ...
                                       style: const TextStyle(
                                         ...
                                         fontSize: 16 // Dynamic font size could be better but sticking to fixed for now
                                       ),
                                     ),
                                   ),
                                   Expanded(
